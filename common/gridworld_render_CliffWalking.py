...
class Renderer:
    def __init__(self, reward_map, goal_state, wall_state, cliff_states):
    ...
    def set_figure(self, figsize=None):
        ...
        # 余白調整
        plt.subplots_adjust(left=0.01, right=0.98, bottom=0, top=1)
        ax = self.ax
        ax.clear()
        ...
        for y in range(ys):
            for x in range(xs):
                ...
                if policy is not None and state != self.wall_state:
                    ...
                    arrows = ["↑", "↓", "←", "→"]
                    offsets = [(0, 0.0), (-0.0, -0.22), (-0.15, -0.1), (0.15, -0.1)]
                    for action in max_actions:
                        arrow = arrows[action]
                        offset = offsets[action]
                        if state == self.goal_state or state in self.cliff_states:
                            continue
                        #ax.text(x+0.45+offset[0], ys-y-0.5+offset[1], arrow)
                        ax.text(x+0.3+offset[0], ys-y-0.4+offset[1], arrow, fontsize=30)
                        # 大きい矢印のために調節
                        #ax.text(x+0.3, ys-y-0.6, arrow, fontsize=40)

                if state == self.wall_state or state in self.cliff_states:
    ...
    def render_e(self, e):
        ...
        emax, emin = max(e.values()), min(e.values())
        emax = max(emax, abs(emin))
        emin = -1 * emax
        region = 0.1
        emax = region if emax < region else emax
        emin = -region if emin > -region else emin
        ...
        for y in range(ys):
             for x in range(xs):
                 ...
                 if state == self.wall_state or state in self.cliff_states:
                     ...
                     tq = e[state]
                     color_scale = 0.5 + (tq / emax) / 2  # normalize: 0.0-1.0
                     #color_scale = (tq - emin) / (emax - emin)
                     ax.add_patch(plt.Rectangle((tx, ty), 1, 1, fc=cmap(color_scale)))
                     ax.text(tx+0.1, ty+0.7, "{:10.6f}".format(tq))
        plt.show()

    def render_end_state(self, end_states):
        self.set_figure()
        ...
        for y in range(ys):
            for x in range(xs):
                ...
                if state == self.wall_state or state in self.cliff_states:
                    ...
                    tq = end_states[state]
                    #color_scale = (tq - emin) / (emax - emin)
                    ax.add_patch(plt.Rectangle((tx, ty), 1, 1, fc=(1,1,1,1)))
                    ax.text(tx+0.1, ty+0.7, "{}".format(tq))
        plt.show()